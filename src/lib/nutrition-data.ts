...
// Get Phase 2 allowed day check
function isPhase2AllowedDay(): boolean {
  const day = new Date().getDay(); // 0=Sun, 1=Mon, ..., 6=Sat
  return day === 3 || day === 6 || day === 0; // Wed=3, Sat=6, Sun=0
}

export interface MealSuggestion {
  meal: string;       // e.g. "6oz grilled chicken breast"
  veggies: string;    // e.g. "2 cups broccoli"
  fat: string;        // e.g. "2 tbsp olive oil"
  starch?: string;    // e.g. "2 cups rice" — undefined if not allowed
  message: string;    // short coaching message
}

/**
 * Generate a rule-based meal suggestion for any phase/gender combination.
 * Portions come from getPortions(), foods come from Allen's exact food lists.
 * AI ONLY provides the coaching tone — never invents portions or foods.
 */
export function generateMealSuggestion(
  ...
  // Pick random protein (exclude whey/bacon from main meals — those are supplements)
  ...
  // Pick fat
  const fatOptions = HEALTHY_FATS.filter(f => !f.includes('MCT')); // MCT is coffee-only
  ...
  if (starchAllowed) {
    // Pick a starch from Allen's list
...
/**
 * Format a MealSuggestion as a short punchy coaching message.
 * This is what the client sees — all amounts are from code, not AI.
 */
export function formatMealSuggestion(suggestion: MealSuggestion): string {
  let response = `${suggestion.meal}, ${suggestion.veggies}`;
  
  if (suggestion.starch) {
    response += `, ${suggestion.starch}`;
  }
  ...
  switch (phase) {
    ...
    case 6:
      return {
        ...
        water: p.water,
        supplements: [
          `Whey protein: ${isMale ? '40g × 2/day' : '20g × 2/day'} (first thing AM + last thing before bed)`,
