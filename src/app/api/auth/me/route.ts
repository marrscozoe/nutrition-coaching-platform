...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data, error } = await supabase
      .from('clients')
      .select('id, trainer_id, email, name, gender, program_type, starting_weight, current_weight, goal_weight, goal_start_date, goal_type, event_date, current_phase, current_week, phase5_plan, phase5_start_date, waiver_signed, subscription_status, subscription_end_date, notes, lead_source, allergies, custom_allergy_bans, allergy_discovery_enabled, photo_meal_log_enabled, created_at, updated_at')
      .eq('id', clientId)
      .single();
