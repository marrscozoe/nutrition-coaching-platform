...
assert(resultD.fatTbsp === 1, 'meal fat should be exactly 1');
...
console.log('  fatTbsp:', reg2.fatTbsp, '(expected: 1 — unitless fraction is pre-existing limitation)');
assertApprox(reg2.proteinOz, 2, 'proteinOz');
// NOTE: 1/2 avocado without unit defaults to 1 tbsp (unitless fraction is a pre-existing limitation)
assertApprox(reg2.fatTbsp, 1, 'fatTbsp');
...
assert(plainWaterCoffee === true, 'plainWater should be true');
